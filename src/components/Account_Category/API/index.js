...
export const callAPI_deleteByID = async (id) => {
  ...
  try {
    const response = await axios.delete(paths.CATEGORY, { data: { id: id } });
    return response.data;
  } catch (error) {
    throw error;
  }
...
export const callApi_createCategory = async (name, group_id) => {
  try {
...
export const callApi_editCategory = async (id, name, group_id) => {
  try {
